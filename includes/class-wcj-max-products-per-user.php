...
if ( ! class_exists( 'WCJ_Max_products_Per_User' ) ) :
	...
	class WCJ_Max_products_Per_User extends WCJ_Module {
		...
		public function calculate_data_notice() {
			...
			if ( isset( $_GET['wcj_max_products_per_user_calculate_data_finished'] ) && wp_verify_nonce( $nonce ) ) {
				$class   = 'notice notice-info';
				$message = __( 'Data re-calculated.', 'woocommerce-jetpack' ) . ' ' .
				/* translators: %s: translation added */
				sprintf( __( '%s order(s) processed.', 'woocommerce-jetpack' ), isset( $_GET['wcj_max_products_per_user_calculate_data_finished'] ) );
		...
		public function calculate_data() {
			$nonce = wp_create_nonce();
			if ( isset( $_GET['wcj_max_products_per_user_calculate_data'] ) && wp_verify_nonce( $nonce ) ) {
				$offset       = 0;
				$block_size   = 512;
				$total_orders = 0;
				while ( true ) {
