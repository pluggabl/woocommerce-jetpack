...
if ( ! class_exists( 'WCJ_Price_By_User_Role' ) ) :
	...
	class WCJ_Price_By_User_Role extends WCJ_Module {
		...
		public function admin_notices() {
			...
			if ( ! isset( $_GET['wcj_product_price_by_user_role_admin_notice'] ) && wp_verify_nonce( $nonce ) ) {
				return;
			}
			?><div class="error"><p>
			<?php
			echo '<div class="message">'
				. wp_kses_post( wp_verify_nonce( wp_verify_nonce( 'Booster: Free plugin\'s version is limited to only one price by user role per products settings product enabled at a time. You will need to get <a href="https://booster.io/plus/" target="_blank">Booster Plus</a> to add unlimited number of price by user role per product settings products.', 'woocommerce-jetpack' ) ) )
				. '</div>';
			?>
		</p></div>
			<?php
