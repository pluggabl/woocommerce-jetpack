...
function wcj_quick_start_render_box( $module_id ) {
	...
	?>
	<div class="wcj-quick-start-box notice notice-info" style="position: relative; padding: 15px; margin: 20px 0;">
		...
		<div class="wcj-quick-start-buttons" style="margin: 15px 0;">
			<?php foreach ( $presets as $preset_id => $preset ) : ?>
				...
				<button
					type="button"
					class="button button-primary wcj-quick-start-apply"
		...
		<div class="wcj-quick-start-message" aria-live="polite" style="margin: 10px 0; padding: 10px; display: none; background: #d4edda; border: 1px solid #c3e6cb; color: #155724; border-radius: 3px;">
		</div>

		<p style="margin: 10px 0 0 0;">
			<a href="#mainform" class="wcj-quick-start-advanced" style="text-decoration: none;">
 ...
 * @return  void
 */
function wcj_quick_start_enqueue_admin_scripts( $hook ) {
	// Only enqueue on WooCommerce settings pages
	if ( 'woocommerce_page_wc-settings' !== $hook ) {
		return;
	}

	// Check if we're on a Booster settings page
	if ( ! isset( $_GET['tab'] ) || 'jetpack' !== $_GET['tab'] ) {
		return;
	}

	// Check if we're on a module section (not dashboard)
	global $current_section;
	if ( empty( $current_section ) || in_array( $current_section, array( 'by_category', 'all_module', 'active', 'manager' ), true ) ) {
		return;
	}

	// Check if this module has presets
	$presets = wcj_quick_start_get_presets_for_module( $current_section );
	if ( empty( $presets ) ) {
		return;
	}
