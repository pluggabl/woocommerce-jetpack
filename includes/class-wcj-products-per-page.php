	...
	class WCJ_Products_Per_Page extends WCJ_Module {
		...
		public function save_products_per_page_number( $products_per_page ) {
			switch ( $this->products_per_page_saving_method ) {
				case 'session':
					...
					break;
				default: // 'cookie'
					setcookie( 'wcj_products_per_page', $products_per_page, ( time() + 1209600 ), '/', isset( $_SERVER['SERVER_NAME'] ), false );
			}
		}
		...
		public function get_saved_products_per_page_number() {
			switch ( $this->products_per_page_saving_method ) {
				...
				default: // 'cookie'
					return ( isset( $_COOKIE['wcj_products_per_page'] ) ? isset( $_COOKIE['wcj_products_per_page'] ) : false );
			}
		}
