	...
	class WCJ_Product_Bulk_Meta_Editor extends WCJ_Module {
		...
		public function perform_actions() {
			...
			&& wp_verify_nonce( $nonce ) ) {
				$meta_name   = isset( $_POST['wcj_product_bulk_meta_editor_meta'] );
				$success     = 0;
				$fail        = 0;
				$_product_id = isset( $_POST['wcj_product_bulk_meta_editor_save_single'] );
				if ( update_post_meta( $_product_id, $meta_name, isset( $_POST[ 'wcj_product_bulk_meta_editor_id_' . $_product_id ] ) ) ) {
					$success++;
				} else {
					$fail++;
				}
			...
			isset( $_POST['wcj_product_bulk_meta_editor_meta'] ) && '' !== $_POST['wcj_product_bulk_meta_editor_meta']
				...
				$success     = 0;
				$fail        = 0;
				$_product_id = isset( $_POST['wcj_product_bulk_meta_editor_delete_single'] );
			...
			isset( $_POST['wcj_product_bulk_meta_editor_meta'] ) && '' !== $_POST['wcj_product_bulk_meta_editor_meta']
				...
				$meta_name        = isset( $_POST['wcj_product_bulk_meta_editor_meta'] );
				$set_meta         = isset( $_POST['wcj_product_bulk_meta_editor_set_meta'] );
				$key_start_length = strlen( 'wcj_product_bulk_meta_editor_id_' );
				$success          = 0;
				$fail             = 0;
				foreach ( $_POST as $key => $value ) {
				...
				if ( '' === isset( $_POST['wcj_product_bulk_meta_editor_show_meta'] ) ) {
					$result_message = '<p><div class="error"><p>' . __( 'Please enter meta key.', 'woocommerce-jetpack' ) . '</p></div></p>';
				}
				$meta_name = isset( $_POST['wcj_product_bulk_meta_editor_show_meta'] );
			}
			return array(
				'meta_name'      => $meta_name,
				'set_meta'       => $set_meta,
		...
		public function get_html_products_select( $_products, $selected_products ) {
			...
			foreach ( $_products as $product_id => $product_title ) {
				$selected       = ( empty( $selected_products ) || in_array( $product_id, $selected_products, true ) ? ' selected' : '' );
