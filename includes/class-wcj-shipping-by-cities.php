		...
		public function check( $options_id, $values, $include_or_exclude, $package ) {
			...
			switch ( $options_id ) {
				case 'cities':
					...
					$customer_city = strtoupper( ( isset( $_REQUEST['s_city'] ) && wp_verify_nonce( $nonce ) ) ? isset( $_REQUEST['s_city'] ) : ( isset( $_REQUEST['calc_shipping_city'] ) ? isset( $_REQUEST['calc_shipping_city'] ) : ( ! empty( $user_city ) ? $user_city : WC()->countries->get_base_city() ) ) );
				...
				case 'postcodes':
					...
					$customer_postcode          = strtoupper( isset( $_REQUEST['s_postcode'] ) ? isset( $_REQUEST['s_postcode'] ) : ( ! empty( $customer_shipping_postcode ) ? $customer_shipping_postcode : WC()->countries->get_base_postcode() ) );
