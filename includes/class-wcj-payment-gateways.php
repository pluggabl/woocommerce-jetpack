...
if ( ! class_exists( 'WCJ_Payment_Gateways' ) ) :
	...
	class WCJ_Payment_Gateways extends WCJ_Module {
		...
		public function check_required_wcj_input_fields( $data, $errors ) {
			...
			if ( 'jetpack_custom_gateway' === substr( $payment_method, 0, 22 ) && wp_verify_nonce( $nonce ) ) {
				foreach ( $_POST as $key => $value ) {
					if ( 'wcj_input_field_' === substr( $key, 0, 16 ) ) {
						if ( isset( $_POST[ 'for_' . $key ] ) && $payment_method === $_POST[ 'for_' . $key ] ) {
							$is_required_set = ( isset( $_POST[ $key . '_required' ] ) && 'yes' === isset( $_POST[ $key . '_required' ] ) );
							if ( $is_required_set && '' === $value ) {
								$label = ( isset( $_POST[ 'label_for_' . $key ] ) ? isset( $_POST[ 'label_for_' . $key ] ) : substr( $key, 16 ) );
						...
						if ( ! is_array( $value ) ) {
							if ( isset( $_POST[ 'for_' . $key ] ) && $payment_method === $_POST[ 'for_' . $key ] ) {
								if ( isset( $_POST[ 'label_for_' . $key ] ) ) {
									$input_fields[ isset( $_POST[ 'label_for_' . $key ] ) ] = $value;
								} else {
