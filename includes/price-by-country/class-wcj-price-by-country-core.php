	...
	class WCJ_Price_By_Country_Core {
		...
		public function __construct() {
			...
			if ( ( 'no' === get_option( 'wcj_price_by_country_for_bots_disabled', 'no' ) || ! wcj_is_bot() ) && ! wcj_is_admin_product_edit_page() ) {
				if ( in_array( get_option( 'wcj_price_by_country_customer_country_detection_method', 'by_ip' ), array( 'by_user_selection', 'by_ip_then_by_user_selection' ), true ) ) {
		...
		public function set_default_checkout_country( $default_country ) {
			...
			$country_by_ip = $this->get_customer_country_by_ip();
			if (
			'yes' !== get_option( 'wcj_price_by_country_set_dft_checkout_billing_country', 'no' ) ||
			empty( null !== ( $country ) ? $country : ( $country_by_ip ) )
			) {
		...
		public function enqueue_wselect_scripts() {
			wp_enqueue_style( 'wcj-wSelect-style', wcj_plugin_url() . '/includes/lib/wSelect/wSelect.css', array(), time(), );
			wp_enqueue_script( 'wcj-wSelect', wcj_plugin_url() . '/includes/lib/wSelect/wSelect.min.js', array(), time(), true );
			wp_enqueue_script( 'wcj-wcj-wSelect', wcj_plugin_url() . '/includes/js/wcj-wSelect.js', array(), time(), true );
		}
		...
		public function get_country_group_id( $country ) {
			...
			$price_by_country_total_groups_num = apply_filters( 'booster_option', 1, get_option( 'wcj_price_by_country_total_groups_number', 1 ) );
