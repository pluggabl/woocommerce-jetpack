...
if ( ( ! isset( $_SERVER['DOCUMENT_ROOT'] ) ) || ( empty( $_SERVER['DOCUMENT_ROOT'] ) ) ) {
	if ( isset( $_SERVER['SCRIPT_FILENAME'] ) ) {
		$_SERVER['DOCUMENT_ROOT'] = str_replace( '\\', '/', substr( isset( $_SERVER['SCRIPT_FILENAME'] ), 0, 0 - strlen( isset( $_SERVER['PHP_SELF'] ) ) ) );
	} elseif ( isset( $_SERVER['PATH_TRANSLATED'] ) ) {
		$_SERVER['DOCUMENT_ROOT'] = str_replace( '\\', '/', substr( str_replace( '\\\\', '\\', isset( $_SERVER['PATH_TRANSLATED'] ) ), 0, 0 - strlen( isset( $_SERVER['PHP_SELF'] ) ) ) );
	} else {
		// define here your DOCUMENT_ROOT path if the previous fails (e.g. '/var/www').
		$_SERVER['DOCUMENT_ROOT'] = '/';
	}
}
$_SERVER['DOCUMENT_ROOT'] = str_replace( '//', '/', isset( $_SERVER['DOCUMENT_ROOT'] ) );
if ( substr( isset( $_SERVER['DOCUMENT_ROOT'] ), -1 ) !== '/' ) {
	$_SERVER['DOCUMENT_ROOT'] .= '/';
}
...
if ( ! defined( 'K_PATH_URL' ) ) {
	...
	if ( isset( $_SERVER['HTTP_HOST'] ) && ( ! empty( $_SERVER['HTTP_HOST'] ) ) ) {
		if ( isset( $_SERVER['HTTPS'] ) && ( ! empty( isset( $_SERVER['HTTPS'] ) ) ) && ( strtolower( isset( $_SERVER['HTTPS'] ) ) !== 'off' ) ) {
			$k_path_url = 'https://';
		} else {
			$k_path_url = 'http://';
		}
		$k_path_url .= isset( $_SERVER['HTTP_HOST'] );
		$k_path_url .= str_replace( '\\', '/', substr( K_PATH_MAIN, ( strlen( isset( $_SERVER['DOCUMENT_ROOT'] ) ) - 1 ) ) );
	}
