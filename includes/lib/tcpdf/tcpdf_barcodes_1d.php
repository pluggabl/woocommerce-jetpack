...
class TCPDFBarcode {
	...
	protected function checksum_code39( $code ) {
		...
		$clen  = strlen( $code );
		for ( $i = 0; $i < $clen; ++$i ) {
			$k    = array_keys( $chars, $code[ $i ], true );
			$sum += $k[0];
		}
		$j = ( $sum % 43 );
		return $chars[ $j ];
	...
	protected function binseq_to_array( $seq, $bararray ) {
		...
		for ( $i = 0; $i < $len; ++$i ) {
			$w += 1;
			if ( ( ( $len - 1 ) === $i ) || ( ( $i < ( $len - 1 ) ) && ( $seq[ $i ] !== $seq[ ( $i + 1 ) ] ) ) ) {
				if ( '1' === $seq[ $i ] ) {
					$t = true; // bar.
				} else {
					$t = false; // space.
	...
	protected function barcode_eanupc( $code, $len = 13 ) {
		...
		for ( $i = 0; $i < $clen; ++$i ) {
			$w += 1;
			if ( ( ( $clen - 1 ) === $i ) || ( ( $i < ( $clen - 1 ) ) && ( $seq[ $i ] !== $seq[ $i + 1 ] ) ) ) {
				if ( '1' === $seq[ $i ] ) {
					$t = true; // bar.
				} else {
					$t = false; // space.
	...
	protected function barcode_rms4cc( $code, $kix = false ) {
		...
		if ( $notkix ) {
			...
			$row  %= 6;
			$col  %= 6;
			$chk   = array_keys( $checktable, array( $row, $col ), true );
			$code .= $chk[0];
			++$len;
		}
		$k = 0;
