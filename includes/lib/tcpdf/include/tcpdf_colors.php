 ...
 * Version     : 1.0.004
 * Begin       : 2002-04-09/ Last Update : 2014-04-25
...
class TCPDF_COLORS {
	...
	public static function convertHTMLColorToDec( $hcolor, &$spotc, $defcol = array(
		...
		if ( strpos( $color, '[' ) !== false ) {
			...
			$dotpos = strpos( $color, '.' );
		} elseif ( ( substr( $color, 0, 4 ) !== 'cmyk' ) && ( substr( $color, 0, 3 ) !== 'rgb' ) && ( ( $dotpos ) !== false ) ) {
	...
	public static function _JScolor( $color ) {
		...
		if ( ! in_array( $color, self::$jscolor, true ) ) {
